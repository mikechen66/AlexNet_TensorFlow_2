...
def img2PIL(image):
    """
    Converts an image to a pillow object. If it is greyscale image, please convert it to 
    RGB first
...
def img2np(image, size=None):
    """
    Converts an image to numpy data. If it is greyscale image, please convert it to RGB 
    first and then change it to a numpy array
    :param image: path to the image file
...
def gen_mean_activity(base_dir):
    """
    Generate mean activity for each channel over the entire train dataset
...
def resize(img):
    """
    Resize the image to 256 x 256. Rescale the image such that the shorter side will be
    ...
    # Change the mode to RGB if necessary
    if img.mode != 'RGB':
        img = img.convert('RGB')
...
class Store:
    """
    A store to keep batches of data for deep learning using threading.
    """
    def __init__(self, source, max_qsize):
        """
        :param source: It tell how to get the data. This is a tuple(tuple==>(function, int, int))
         of function, total size of data for one epoch and batch size. The function is used to get 
         the data to store 
        ...
        self.logger = logging.getLogger('utils.Store')
    ...
    def write(self):
        """
        Store datas by the function given using threading. It read sdatas from disk parallelly.
        """
        threads = []
    ...
    def read(self):
        """
        Generator to read data from the store. Creates a generator to read data from store(not disk).
        It first starts reading the data from disk using threading so that the data in the store is 
        always available while reading.
        """
